...
import { GLOBAL_ADMIN_USER_ID, GLOBAL_USERS_NODE_ID, USER_ROOT_ID_PREFIX } from "@/lib/constants";
...
async function migrateRelationTypes() {
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      const allRelationTypes = await tx.select().from(relationTypeTable); // PersistedRelationType[]
      const allRelations = await tx.select().from(graphRelationTable); // PersistedGraphRelation[]

      // 2) For convenience, store all non-default relation types keyed by their ID:
      const nonDefaultRelationTypes = allRelationTypes.filter(
        (rt) => !Object.keys(defaultRelationTypes).includes(rt.id),
      );
      ...
      for (const user of users) {
        ...
        if (!userRootNode) {
          ...
          const rootId = `${USER_ROOT_ID_PREFIX}${userId}`; // or something else
          const [insertedRoot] = await tx
            .insert(graphNodeTable)
            .values({
              id: rootId,
              authorId: userId,
              content: JSON.stringify([{ type: "text", value: userId }]),
            })
            .returning();
        ...
        let userRelationTypesNode = await tx.query.graphNodeTable.findFirst({
          where: (fields, { eq }) => eq(fields.id, desiredRelationTypesNodeId),
        });
        if (!userRelationTypesNode) {
          const [insertedRelationTypes] = await tx
            .insert(graphNodeTable)
            .values({
      ...
      for (const [i, rt] of nonDefaultRelationTypes.entries()) {
        ...
        if (!parentRelationTypesNodeId) {
          console.warn(
            `Could not find "Relation Types" node for user ${authorId}. Skipping relation_type ${relationTypeId} with label ${label}`,
          );
          continue;
        }
        ...
        await tx.delete(relationTypeTable).where(eq(relationTypeTable.id, relationTypeId));
      }
      ...
      for (const [i, rel] of updateCandidates.entries()) {
        if (i % 100 === 0) {
          console.log(i);
        }
        ...
        await tx.update(graphRelationTable).set({ relationTypeId: "child" }).where(eq(graphRelationTable.pk, rel.pk));
