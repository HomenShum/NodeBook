...
An unchanged user retry reuses one UUID request identity and therefore one canonical trace. Each structured provider call receives a deterministic SHA-256 step key over its bounded semantic input. A completed owner-scoped Convex journal entry is replayed before any new provider call. Changed query, mode, execution mode, web-research setting, or notebook root produces a fresh request identity.
...
This closes duplicate billing for every already-journaled completed step. It does not eliminate the narrow crash window between receiving a provider response and persisting that response to Convex. A pre-provider durable claim/lease is still required before describing the system as strictly exactly-once across process crashes.

## Verification

- Jest: 53 suites, 266 tests passed.
- Convex production contracts: 48 tests passed.
