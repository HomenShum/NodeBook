...
Modernization inside this boundary: Place a compact receipt-backed React Flow map and summary strip at the top of the disclosure. NodeAgent is the center node and the six locked cases are the only surrounding nodes. An edge is neutral before execution, highlighted only while that exact case is running, teal only after a durable PASS receipt, and tomato only after a durable FAIL receipt. The exact case list remains the accessible text fallback and evidence surface. No decorative or inferred activity is permitted. The renderer is `@xyflow/react`; NodeBook does not own a custom SVG graph renderer.
