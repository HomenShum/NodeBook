...
import { createContext, useContext, useEffect, useCallback, useRef, useState, ReactNode, MouseEvent, SetStateAction } from "react";
...
const FORMAT_OPTIONS = [
  ...
  { 
    ...
    icon: <Type size={14} style={{ color: 'var(--gray-11)' }} />, 
    label: "Uppercase", 
    hotkey: "" 
  },
  { 
    format: "lowercase" as const, 
    icon: <ALargeSmall size={14} style={{ color: 'var(--gray-11)' }} />, 
    label: "Lowercase", 
    hotkey: "" 
  },
];
...
function FormattingMenu(): JSX.Element | null {
  ...
  const onUpdatePosition = useCallback((range: Range) => {
  ...
  const onFormatClick = useCallback((format: TextFormatType, e: MouseEvent) => {
          ...
          <div className={styles.FormattingMenuInner}>
            {FORMAT_OPTIONS.map(({ format, icon, label, hotkey }) => (
              <button
                ...
                className={cn(styles.FormatButton, selectedFormats.has(format) && styles.active)}
