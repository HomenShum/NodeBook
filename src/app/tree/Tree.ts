...
import {
  extractGroupId,
  extractPointedAtObjectId,
  getCanonicalPath,
  getSideOrThrow,
  sliceChips,
} from "@/app/graph/utils";
...
export class Tree {
  constructor(
    ...
    const { rootObjectId, pathToRootIds } = this.setRoot(root);
  ...
  public get pathToRoot(): { relation: GraphRelation | undefined; childGroupId: GroupId }[] {
    return this.pathToRootIds.map(({ relationId, childGroupId }) => ({
   ...
   *
   * The root can be specified in a few different ways:
   * - TreeNode -> use it's ancestors as the path
   * - GraphObject -> use it's canonical path
   * - ObjectPath -> use it as the path
   *
  ...
  setRoot(root: Root) {
    ...
    if (root instanceof DescendantTreeNode) {
      ...
      this.pathToRootIds = getAncestorsAsArray(root).map((node) => ({
        relationId: node.relationToChild.id,
        childGroupId: node.childGroupId,
      }));
    } else if (isGraphObject(root)) {
      const relations = getCanonicalPath(root).relations || [];
      this.rootObjectId = root.id;
      this.pathToRootIds = relations.map((r) => ({
        relationId: r.id,
        childGroupId: "all",
      }));
    } else {
  ...
  *traverseNodePaths(node: TreeNode, seenIDs: Set<string> = new Set<string>()): Generator<string> {
  ...
  async split(treeNode: TreeNode, chips?: { before: Chip[]; after: Chip[] }) {
    ...
    this.graphStore.applyCombinedTransaction(changes.txs);
  ...
  /** Helper class to manage server-side expansion states */
  private expansionStateManager: ExpansionStateManager;

  /** Tracks object IDs that should be expanded when they're loaded */
  ...
  /**
   * Clears the expansion state for the current root object
   */
  public async clearExpansionStateForAllUsers(): Promise<boolean> {
    if (!this.rootObjectId) return false;

    logger.debug(`Clearing expansion state for root: ${this.rootObjectId}`);
