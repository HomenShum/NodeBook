...
3. **Completed-step replay journal (implemented; crash-window closure pending):** the client reuses one request/trace identity for unchanged retries, provider inputs receive deterministic step keys, completed outputs are owner-scoped and replayed without another model call, failed runs can recover in place, and history is bounded to 100 steps per trace / 500 per owner. First-writer canonicalization handles simultaneous completions, but a process crash after the provider responds and before the Convex write can still rebill after retry; a pre-call lease/claim is the remaining strict exactly-once closure.
