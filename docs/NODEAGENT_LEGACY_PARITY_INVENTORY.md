...
| Company/person deep dives and report outline/population were replaced by generic research | **Corrected in code; production proof pending.** Research-profile commands now dispatch to bounded web research before the generic profile-clone path, carry the entity plan into synthesis, require every person/company aspect, and deterministically repair underspecified drafts before exposing typed operations. |
...
| `research_person_deep_dive` | Both | Replaced / production proof pending | Up to six bounded parallel searches feed a professional-profile synthesis; `research/deep dive profile` requests no longer fall into the non-research clone path. |
