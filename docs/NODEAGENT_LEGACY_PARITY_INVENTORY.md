...
7. A signed one-node plan exposed a model-quality regression: four semantically identical `run_specialized_workflow` calls with paraphrased rationales consumed 27,459 tokens. The repeat guard now keys only semantic tool identity, and checkpoint-quality failures feed the existing failure-triggered model benchmark without treating degraded semantic retrieval as a model failure. Renewed production observation remains required after deployment.
8. Signed snapshot hydration exposed repeated inaccessible legacy relation-list references. The complete eight-stream Convex read boundary now prunes only positions whose relation document is absent from the owner-visible snapshot; renewed production console proof remains required after deployment.
