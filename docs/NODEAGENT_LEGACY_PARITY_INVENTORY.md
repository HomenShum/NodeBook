...
| Company/person deep dives and report outline/population were replaced by generic research | **Corrected; person flow production-proven, company flow production proof pending.** Research-profile commands now dispatch to bounded web research before the generic profile-clone path, carry the entity plan into synthesis, require every person/company aspect, and deterministically repair underspecified drafts before exposing typed operations. |
...
| `research_company_deep_dive` | Both | Replaced / production proof pending | Up to six bounded parallel searches feed a company-specific five-aspect synthesis; drafts with fewer sections fail deterministic validation and receive one bounded repair. |
...
6. Person deep-dive parity is authenticated-production-proven: a five-aspect request was classified as `person`, completed five of six bounded searches, applied a seven-operation professional-profile hierarchy, and was absent after durable Undo plus reload. The company variant remains scenario-proven, including deterministic repair of five unrelated sections, but still needs one signed production receipt.
