...
This is the durable migration ledger for the in-place MewAgent-to-NodeAgent port. It inventories both recoverable legacy generations and records whether each behavior is ported, intentionally replaced, or was inactive historical code. Historical names appear here only as provenance; shipped product copy and commands use NodeBook and NodeAgent.
...
|---|---|
| Specialized research/organization/profile behavior was ported through instructions | Until 2026-08-02, `run_specialized_workflow` returned only candidate IDs and the final model had to invent all operation structure. Research now generates 4–6 bounded targeted queries, runs them in parallel, records honest evidence receipts, and synthesizes 3–12 typed hierarchical work products; scenario tests and renewed signed production receipts pass. |
...
| Rename followed parity | **Historically contradicted.** Product code said NodeAgent before behavioral parity was proven; sequencing cannot be retroactively changed. Current production parity is now proven, so this remains a chronology note rather than an open product gap. |
...
- **Inactive historical**: present only as commented or unreachable legacy source and never an active capability.
...
| `research_and_create_structured_report` | Google, commented out | Inactive historical | The declaration and implementation were commented out in the preserved checkout. Its intended container-first report behavior is covered by the production-proven multi-step research workflow, but it is not counted as an active legacy tool. |
...
| `search_academic` | OpenAI | Replaced | Legacy code called the same generic search endpoint as `web_search` and merely added `source: "academic"`; it did not query or verify a specialist index. Current cited web research preserves the real behavior without a false source claim. |
| `search_news` | OpenAI | Replaced | Legacy code called the same generic search endpoint as `web_search` and merely added `source: "news"`; it did not apply a news index or historical filter. Current cited web research preserves the real behavior without a false source claim. |
...
| `superconnector_prepare_outreach` | OpenAI | Replaced | The legacy "foundational stub" only emitted a client action and returned `{success: true}`; it sent no outreach. NodeAgent can draft outreach read-only through `execute_direct_request`, but does not claim delivery without an authenticated integration and receipt. |
| `superconnector_schedule_event` | OpenAI | Replaced | The legacy "foundational stub" only emitted a client action and returned `{success: true}`; it scheduled nothing. NodeAgent intentionally fails closed instead of advertising an unconnected calendar mutation. |
...
| SOP B: deep knowledge organization | Create one container before children and preserve hierarchy | Production-proven by the signed Web3 and person/company runs: each emitted one ordered container-first operation contract, survived reload, and was fully removed by whole-run Undo plus clean reload. |
...
Implementation is not behavioral proof. The authenticated production evidence set covers:
...
4. The final authenticated desktop proof covers progressive trace-before-receipt behavior. Fresh phone 390x844 and tablet 768x1024 clips from deployment `dpl_6KjTw7so3SXzbXyJ7Y8CvYewqaVe` cover signed Ask, safe Auto apply, durable receipt, Undo, and clean reload with exact viewport/overflow and zero-console-error checks.
